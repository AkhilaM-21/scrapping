...
from y import main as run_youtube_scraper
...
async def schedule_daily_youtube_scrape():
    """
    Schedules the YouTube scraper to run daily at 1 AM IST.
    """
    while True:
        try:
            ...
            logger.info(f"Next YouTube scrape scheduled for {target_time} IST (in {wait_seconds:.2f} seconds)")
            
            await asyncio.sleep(wait_seconds)
            
            logger.info("Starting scheduled YouTube scrape...")
            await asyncio.to_thread(run_youtube_scraper)
            logger.info("Scheduled YouTube scrape completed.")
        except Exception as e:
            logger.error(f"Error in YouTube scrape scheduler: {e}")
...
async def startup_event():
    try:
        ...
        asyncio.create_task(schedule_daily_youtube_scrape())
